...
from dataset.helpers import BLACK, COLOR_PALETTE, FULL_CROP, GRAY_75, WHITE, absolute_bbox, additional_parameters_string, filter_annotations, get_circle_size, get_plot_font_size, horizontally_flip_bbox, pad_list, rescale_annotations
from dataset.models import Annotation, BoundingBox
...
class ObjectsCenterPointsConditionalBuilder:
    def __init__(self, categories, no_object_classes: int, no_max_objects: int, no_tokens: int, encode_crop: bool,
    ...
    def tokenize_coordinates(self, x: float, y: float) -> int:
        ...
        from chatgpt: This method converts 2D coordinates (x, y) into a single integer token.
        The method assumes that both x and y are normalized between 0 and 1.
        The space is divided into a grid of self.no_sections by self.no_sections. Each cell in this grid is assigned a unique token.
        It calculates the discrete equivalent of x and y by multiplying them with self.no_sections - 1 and rounding. This maps the continuous coordinates to one of the grid cells.
        The token is calculated by y_discrete * self.no_sections + x_discrete, effectively flattening the 2D grid into a 1D array of tokens.
        """
    ...
    def coordinates_from_token(self, token: int) -> (float, float):
        """
        This is the inverse of tokenize_coordinates. It converts a token back into 2D coordinates.
        It uses modulo (%) and integer division (//) to find the x and y positions in the grid corresponding to the token.
        The coordinates are normalized by dividing them by self.no_sections - 1.
        """
    ...
    def bbox_from_token_pair(self, token1: int, token2: int) -> BoundingBox:
        """
        This method constructs a bounding box from two tokens.
        It assumes that token1 and token2 represent the top-left and bottom-right corners of the bounding box, respectively.
        It first converts these tokens back into 2D coordinates.
        The bounding box is then formed with these coordinates, calculating width and height as x1 - x0 and y1 - y0.
        """
    ...
    def token_pair_from_bbox(self, bbox: BoundingBox) -> Tuple[int, int]:
        """
        This method is the inverse of bbox_from_token_pair. It converts a bounding box into a pair of tokens.
        It tokenizes the coordinates of the top-left corner (x0, y0) and the bottom-right corner (x0 + width, y0 + height) of the bounding box.
        It returns these tokens as a tuple.
        """
    ...
    def plot(self, conditional: LongTensor, category_number_to_id_dict, figure_size: Tuple[int, int],
        ...
        for (representation, (x, y)), color in zip(description, cycle(COLOR_PALETTE)):
            ...
            label = self.categories[category_number_to_id_dict[ann.category_no]].name + ' ' + additional_parameters_string(ann)
...
class ObjectsBoundingBoxConditionalBuilder(ObjectsCenterPointsConditionalBuilder):
    ...
    def plot(self, conditional: LongTensor, category_number_to_id_dict, figure_size: Tuple[int, int],
        ...
        for (representation, bbox), color in zip(description, cycle(COLOR_PALETTE)):
            ...
            class_label = self.categories[category_number_to_id_dict[annotation.category_no]].name + ' ' + additional_parameters_string(annotation)
